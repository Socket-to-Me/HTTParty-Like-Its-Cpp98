...
const std::string& irc::numerics::rpl_yourhost_002(irc::connection& conn) {
    ...
    conn.setmsg("002 " + conn.getnick() + " :Your host is " + irc::server.getname() + ", running version ", irc::server.getversion())
    return conn.getmsg();
}

const std::string& irc::numerics::rpl_created_003(irc::connection& conn) {

    conn.setmsg("003 " + conn.getnick() + " :This server was created " + irc::server.getcreation())
    return conn.getmsg();  
}
